...
class PassageCell: UITableViewCell, UITextViewDelegate {
    ...
    func initWith(title: String, passage: String) {
        let passageText: String = title + "\n\n" + passage
        passageTextView.text = passageText
        let multiAttribute = NSMutableAttributedString(string: passageText, attributes: [NSFontAttributeName: UIFont.systemFontOfSize(14)])
        multiAttribute.addAttribute(NSFontAttributeName, value: UIFont.boldSystemFontOfSize(15), range: NSRange(location: 0, length: title.characters.count))
